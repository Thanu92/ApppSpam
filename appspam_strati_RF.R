...
files <- list.files(path="/home/thanu/Desktop/FishData/New_dataset/STRATIFIED/Appspam_strati", pattern="app[0-9]{1,2}_[0-9]{1,2}S.nwk", full.names=TRUE, recursive=FALSE)
...
names(treeList) <- gsub("/home/thanu/Desktop/FishData/New_dataset/STRATIFIED/Appspam_strati/", "", files)
...
bp <- ggboxplot(RF_metric,x="sample_completeness",y="RF_distance", color = "sample_completeness",palette = c("#6BAED6" ,"#4292C6", "#2171B5", "#084594","#003366")
                ,main="Boxplot of RF distance for Stratified samples based on sample completeness using AppSPAM",add= "point", width=0.8,outlier.shape = NA)
